...
type flowStates = 'start' | 'document' | 'camera' | 'frontSide' | 'liveliness';
    ...
    return <div className='container w-full h-full backdrop-blur-3xl'>
        <div className="h-(--screenHeight) flex h-[--screenHeight] items-center justify-center md:p-2" data-testid="start-kyc-welcome" id="start-kyc-welcome"
            ...
            <div className="overflow-hidden p-0 shadow-card flex size-full w-fit flex-col rounded-none bg-transparent md:max-h-fit md:overflow-hidden md:rounded-pnl-xl" data-testid="start-kyc-welcome" id="start-kyc-welcome">
                ...
                {
                    flow === "camera" && <CameraView setFlow={setFlow} />
                }
                {
                    flow === 'frontSide' && <CameraFeed />
                }
...
const CameraView = ({ setFlow }: { setFlow: (flow: flowStates) => void }) => {
    ...
    return <div className="flex flex-col gap-2 w-full px-2 py-2 h-full">
        <div className="mx-auto flex w-full max-w-md justify-center pt-6 md:pt-8 [@media(max-height:700px)]:pt-4">
...
const StartKycWelcome = ({ setFlow }: { setFlow: (flow: flowStates) => void }) => {
...
const DocumentStep = ({ setFlow }: { setFlow: (flow: flowStates) => void }) => {
